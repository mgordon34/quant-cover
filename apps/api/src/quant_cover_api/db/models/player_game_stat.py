...
class PlayerGameStat(TimestampMixin, Base):
    ...
    team_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("teams.id", ondelete="RESTRICT"), index=True)
    opponent_team_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("teams.id", ondelete="RESTRICT"), index=True)
